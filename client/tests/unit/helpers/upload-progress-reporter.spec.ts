...
describe("UploadProgressReporter", () => {
  describe.each([
    ...
    test("calculates transaction uploadedFraction", () => {
      ...
      const callback = uploadProgressReporter.createProgressReporter((progressReport: ProgressReport) => {
        reportedProgress = progressReport;
      });

      callback({ loaded: loadedBytes, total: totalBytes });
  ...
  describe.each([
   ...
   test("calculates data rate from elapsed time", () => {
      ...
      callback({ loaded: transferredBytes, total: transferredBytes + 1 });
